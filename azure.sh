...
SBCONNECTIONSTRING="$(az servicebus namespace authorization-rule keys list --resource-group $RGNAME --namespace-name $SERVICBUSNAME --name RootManageSharedAccessKey --query primaryConnectionString) --out tsv"
...
az monitor app-insights component create --resource-group $RGNAME --name $APPINSIGHTSNAME --app-id $APPINSIGHTSNAME --location $LOCATION --retention-time 30
